...
Результаты LWRP, SAP и Advertisement объединяются в [NetworkScanner.cs](src/LivewireBrowser.Core/Discovery/NetworkScanner.cs). Устройства, которые TCP/93-обход почему-то пропустил, но которые видны через Advertisement, тоже попадают в список — в качестве модели используется код `TYPE` из их Advertisement-пакета (заглушка-плейсхолдер используется только если `TYPE` не пришёл).
...
- Запись = `тег(4 ASCII-байта) + тип(1 байт) + значение`. Ширина значения зависит от типа: `0x00`→1 байт (счётчик вложенных полей: `NEST`, `INDI`), `0x01`→4 байта (`INIP`=IP устройства; `TYPE`=4-символьный код модели, например `"NX12"`; `PSID`/`FSID`/`BSID`=ID источника — `FSID` это буквально multicast-адрес `239.192.x.y`/`239.193.x.y` как 4 сырых байта), `0x03`→строка (2-байтная длина + ASCII, дополнено нулями), `0x06`/`0x08`→2 байта, `0x07`→1 байт, `0x09`→8 байт (например `BUSY` — флаг занятости/активности источника).
- Два вида пакетов: короткий периодический "маячок" (~87 байт, `ADVT=2`, без списка каналов) и полный анонс (`ADVT=1`, сотни-тысячи байт) с именем устройства (`ATRN`), кодом модели (`TYPE`) и блоками источников, помеченными динамическими тегами `S001`, `S002`, ... — внутри каждого обычно: `PSID` (номер LW-канала, младшие 2 байта), `FSID` (multicast-адрес), `PSNM` (имя канала — то же название поля, что и в LWRP `SRC`). Часть устройств шлёт "облегчённый" блок для источника без настроенного имени — только `PSID`+`BUSY`, вообще без `FSID`/`PSNM`; в этом случае парсер выводит multicast-адрес из одного `PSID` (та же формула `239.192.<hi>.<lo>`, что и для `FSID`) и подставляет вместо имени `"Channel N"`, а не отбрасывает источник.
...
- Байт типа `0x09` был реальным пробелом: более ранние версии парсера его не распознавали и обрывали разбор остатка пакета на первом теге `BUSY`, молча теряя все источники после него (см. [AdvertisementParser.cs](src/LivewireBrowser.Core/Discovery/AdvertisementParser.cs)).
...
> [!WARNING]
> - LWRP-сканирование подсети по умолчанию ограничено 4096 хостами, начиная с адреса сети (см. `NetworkInterfaceHelper.GetHostAddresses`) — для очень крупных подсетей (например /16) напрямую будет опрошена только часть адресов. Это настраивается в Настройках (`AppSettings.MaxSweepHosts`, вплоть до 65536), если нужно, чтобы сам TCP/93-обход дотягивался до дальних хостов; увеличение лимита примерно пропорционально увеличивает время скана. Устройства, увиденные только через пассивные SAP/Advertisement-рассылки, попадают в список даже вне диапазона обхода и автоматически доопрашиваются напрямую по IP для заполнения класса/каналов (тот же механизм автодоопроса в `NetworkScanner.FullScanAsync`, что и у кнопки "Пересканировать") — так что увеличение лимита в основном важно для устройств, которые вообще не анонсируют себя пассивно.
...
> - Обход подсети использует два разных таймаута: короткий (300 мс) на само TCP/93-подключение — этого достаточно на реальной LAN, где несуществующий хост отвечает RST или недоступен почти мгновенно — и более длинный (800 мс) на сбор ответа каждой команды `VER`/`SRC`/`IP`, потому что устройству с большим числом источников может потребоваться больше пары сотен миллисекунд, чтобы отправить весь многострочный ответ. Обратный DNS-запрос выполняется только для хостов, реально ответивших на TCP/93, а не для всех просканированных адресов — раньше это делалось для всех 4096 адресов независимо от того, слушает ли там вообще что-то, и это был реальный баг, из-за которого обход крупной подсети мог "висеть" минутами уже после того, как все реальные устройства давно ответили.
