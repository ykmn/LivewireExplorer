...
LWRP, SAP, and Advertisement results are merged in [NetworkScanner.cs](src/LivewireBrowser.Core/Discovery/NetworkScanner.cs). Devices that the TCP/93 sweep missed but are visible via Advertisement are still included in the list, using the `TYPE` model code from their Advertisement packet as the device model (falling back to a generic placeholder only if `TYPE` wasn't sent).
...
- Record = `tag (4 ASCII bytes) + type (1 byte) + value`. Value width depends on type: `0x00`→1 byte (nested field count: `NEST`, `INDI`), `0x01`→4 bytes (`INIP`=device IP; `TYPE`=4-char model code, e.g. `"NX12"`; `PSID`/`FSID`/`BSID`=source ID — `FSID` is literally the multicast address `239.192.x.y`/`239.193.x.y` as 4 raw bytes), `0x03`→string (2-byte length + ASCII, null-padded), `0x06`/`0x08`→2 bytes, `0x07`→1 byte, `0x09`→8 bytes (e.g. `BUSY`, a source's busy/active flag).
- Two packet types: a short periodic beacon (~87 bytes, `ADVT=2`, no channel list) and a full announcement (`ADVT=1`, hundreds to thousands of bytes) with the device name (`ATRN`), model code (`TYPE`), and source blocks tagged with dynamic labels `S001`, `S002`, ... — each normally containing `PSID` (LW channel number, lower 2 bytes), `FSID` (multicast address), `PSNM` (channel name — same field name as in LWRP `SRC`). Some devices send a "lightweight" block for a source with no configured name — just `PSID`+`BUSY`, no `FSID`/`PSNM` at all; the parser derives the multicast address from `PSID` alone in that case (same `239.192.<hi>.<lo>` formula as `FSID`) and falls back to `"Channel N"` for the name, instead of dropping the source.
...
- The `0x09` type byte was a real gap: earlier parser versions didn't recognize it and aborted the rest of the packet on the first `BUSY` tag, silently losing every source that followed it (see [AdvertisementParser.cs](src/LivewireBrowser.Core/Discovery/AdvertisementParser.cs)).
...
> [!WARNING]
> - LWRP subnet scanning defaults to a 4096-host cap starting from the network address (see `NetworkInterfaceHelper.GetHostAddresses`) — for very large subnets (e.g. /16) only part of the address space is swept directly. This is configurable in Settings (`AppSettings.MaxSweepHosts`, up to 65536) if you need the TCP/93 sweep itself to reach far-out hosts; raising it increases scan duration roughly proportionally. Devices seen only via passive SAP/Advertisement discovery are listed even when outside the sweep range, and are automatically probed by IP afterward to fill in category/channels (`NetworkScanner.FullScanAsync`'s auto-detail step, same mechanism as the manual "Rescan" button) — so raising the cap mainly matters for devices that don't announce themselves passively at all.
...
> - The subnet sweep uses two separate timeouts: a short one (300 ms) for the initial TCP/93 connect — enough on a real LAN, where a non-existent host either RSTs or is unreachable almost immediately — and a longer one (800 ms) for collecting each `VER`/`SRC`/`IP` command's response, since a device with many sources can take longer than a couple hundred ms to send its full multi-line reply. Reverse-DNS lookups are only attempted for hosts that actually answered on TCP/93, not for every swept address — doing it for all 4096 addresses regardless of whether anything is listening was a real bug that could keep a sweep of a large subnet "running" for minutes past the point where every real device had already answered.
